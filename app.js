...
// --- FAQ toggles: add button behavior to show/hide each answer ---
(function initFAQToggles(){
  try {
    ...
    faqItems.forEach(item => {
      ...
      if(!btn){
        const h4 = item.querySelector('h4') || item.querySelector('strong');
        if(h4){
          const newBtn = document.createElement('button');
          newBtn.className = 'toggle-icon';
          newBtn.type = 'button';
          newBtn.setAttribute('aria-expanded', 'false');
          newBtn.setAttribute('aria-label', 'Mostrar respuesta');
          newBtn.innerText = '+';
          h4.appendChild(newBtn);
        }
      }
  ...
  } catch(err){ console.warn('initFAQToggles error', err); }
})();


// --- Certificate modal: close button and simple form handler ---
(function initCertModal(){
  try {
    ...
    // basic submit handler to use existing generateCertificate if available
    const form = document.getElementById('cert-form');
    if(form){
      form.addEventListener('submit', function(e){
        e.preventDefault();
        if(typeof generateCertificate === 'function'){
          try{ generateCertificate(); }catch(err){ console.warn('generateCertificate error', err); }
        } else {
  ...
  } catch(err){ console.warn('initCertModal error', err); }
})();